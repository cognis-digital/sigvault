...
How **sigvault** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `sigvault` lives in the **Sanctions - ownership - finance** cluster.
...
| from | relation | to |
|---|---|---|
| `sigvault` | JSON findings compose with | [`corpmap`](https://github.com/cognis-digital/corpmap), [`cryptotrace`](https://github.com/cognis-digital/cryptotrace), [`maritimeint`](https://github.com/cognis-digital/maritimeint) |
| `sigvault` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `sigvault` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `sigvault` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the sanctions - ownership - finance cluster.** `sigvault` output feeds the next tool:
```bash
sigvault ... --format json > out.json          # this tool's findings (see `sigvault --help`)
...
sigvault ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
sigvault ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
sigvault ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `sigvault`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `sigvault` slots
