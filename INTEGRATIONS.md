...
**sigvault** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `sigvault` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
sigvault ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
sigvault ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
sigvault ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `sigvault` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="sigvault") for rec in records]   # records = your JSON output
